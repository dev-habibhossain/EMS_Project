...
const canCreate = computed(() => isAdmin.value || permissions.value.includes('products.create'));
const canManage = computed(() => isAdmin.value || permissions.value.includes('products.update') || permissions.value.includes('products.delete'));

// Filter State
const search = ref(props.filters.q ?? '');
const selectedBrand = ref(props.filters.brand_id ? String(props.filters.brand_id) : '');
const selectedSize = ref(props.filters.tile_size_id ? String(props.filters.tile_size_id) : '');
...
<template>
    <div class="flex min-h-full flex-1 flex-col bg-[#F3EFE8] text-[#1C1916]">
        ...
        <header class="border-b border-[#E5E0D8] bg-[#FAF8F5] px-4 py-4 sm:px-6 lg:px-8">
                    ...
                    <div class="flex items-center gap-2.5">
                        ...
                        <div>
                            ...
                            <p class="text-[12px] text-[#6B645B]">
                                Tiles catalog, dimensional conversion factors, warehouse aggregated stocks, and tiered rates
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div class="mt-4 flex flex-wrap items-center gap-3 text-[12px]">
                ...
                <div class="flex items-center gap-1.5 rounded border border-[#E5E0D8] bg-white px-2.5 py-1">
                    ...
                    <span class="font-medium font-mono text-[#1C1916]">{{ products.per_page }}</span>
                </div>
            </div>
        </header>
                ...
                <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                    ...
                    <div class="flex flex-wrap items-center gap-2">
                        ...
                        >
                            ...
                            <option v-for="s in tileSizes" :key="s.id" :value="String(s.id)">
                                {{ s.label }} mm
                            </option>
                        </select>
            ...
            <div class="overflow-hidden rounded border border-[#E5E0D8] bg-white shadow-xs">
                    ...
                    <table class="w-full text-left text-[13px]">
                        <thead>
                            <tr class="border-b border-[#E5E0D8] bg-[#F7F5F0] text-[11px] font-semibold uppercase tracking-wider text-[#6B645B]">
                                <th scope="col" class="py-2.5 pl-4 pr-3">SKU & Barcode</th>
                                <th scope="col" class="px-3 py-2.5">Product & Brand</th>
                                <th scope="col" class="px-3 py-2.5">Size & Origin</th>
                                <th scope="col" class="px-3 py-2.5 text-right">Packaging Unit</th>
                                <th scope="col" class="px-3 py-2.5 text-right">Pricing (৳)</th>
                                <th scope="col" class="px-3 py-2.5 text-right">On-Hand Stock</th>
                        ...
                        <tbody class="divide-y divide-[#EFECE6]">
                            ...
                            <tr v-if="products.data.length === 0">
                                <td :colspan="canManage ? 8 : 8" class="py-12 text-center">
                                    <div class="mx-auto flex max-w-sm flex-col items-center">
                                        ...
                                        <p class="mt-1 text-[13px] text-[#6B645B]">
                                            {{ hasActiveFilters ? 'Try adjusting your search keywords or filter criteria.' : 'No products have been registered in the catalog yet.' }}
                                        </p>
                                        <button
                                            v-if="hasActiveFilters"
                                            type="button"
                            ...
                            >
                                <!-- SKU & Barcode -->
                                <td class="py-3 pl-4 pr-3 align-top font-mono text-[12px]">
                                    <div class="font-semibold text-[#1C1916]">
                                        {{ item.sku }}
                                    </div>
                                    <div v-if="item.barcode" class="mt-0.5 text-[11px] text-[#8F887C]">
                                        {{ item.barcode }}
                                    </div>
                                    <div v-else class="mt-0.5 text-[10px] text-[#A8A196]">
                                        No Barcode
                                    </div>
                                </td>
                                ...
                                <td class="px-3 py-3 align-top">
                                    ...
                                    <div v-if="item.name_bn" class="text-[12px] text-[#6B645B]">
                                        {{ item.name_bn }}
                                    </div>
                                ...
                                <td class="px-3 py-3 align-top text-[12px]">
                                    ...
                                    <div class="mt-0.5 text-[11px] text-[#6B645B]">
                                        {{ item.tile_factory?.name ?? 'Standard' }}
                                    </div>
                                </td>

                                <!-- Packaging Conversion Factors -->
                                <td class="px-3 py-3 text-right align-top tabular-nums font-mono text-[12px]">
                                    <div class="font-medium text-[#1C1916]">
                                        {{ item.pieces_per_box }} <span class="text-[10px] font-normal font-sans text-[#6B645B]">pcs/box</span>
                                    </div>
                                    <div class="mt-0.5 text-[11px] text-[#6B645B]">
                                        {{ formatNumber(item.sqft_per_box, 2) }} <span class="text-[10px] font-normal font-sans">sqft/box</span>
                                    </div>
                                    <div class="mt-0.5 text-[10px] text-[#8F887C]">
                                        ({{ formatNumber(item.sqft_per_piece, 3) }} sqft/pc)
                                    </div>
                                </td>
                                ...
                                <td class="px-3 py-3 text-right align-top tabular-nums font-mono text-[12px]">
                                    <div class="font-semibold text-[#1C1916]">
                                        ...
                                        <span class="text-[10px] font-normal font-sans text-[#6B645B]">/box</span>
                                    </div>
                                    <div class="mt-0.5 text-[11px] text-[#B44422]">
                                        {{ formatCurrency(getSqftRate(item)) }}
                                        <span class="text-[10px] font-normal font-sans text-[#6B645B]">/sqft</span>
                                    </div>
                                </td>
                                ...
                                <td class="px-3 py-3 text-right align-top tabular-nums font-mono text-[12px]">
                                    <div
                                        :class="[
                                            'font-semibold',
                                            getStockTriple(item).isZero ? 'text-rose-600' : 'text-[#1C1916]',
                                        ]"
                                    >
                                    ...
                                    <div class="mt-0.5 text-[11px] text-[#6B645B]">
                                        ...
                                        <span class="text-[10px] font-normal font-sans text-[#8F887C]">PCS</span>
                                    </div>
                                    <div class="mt-0.5 text-[10.5px] text-[#8F887C]">
                                        {{ getStockTriple(item).sqft }}
                                        <span class="text-[10px] font-normal font-sans text-[#A8A196]">SQFT</span>
                                    </div>
                                </td>
                ...
                <div class="flex flex-col gap-3 border-t border-[#E5E0D8] bg-[#FAF8F5] px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div class="flex flex-wrap items-center gap-4 text-[12.5px] text-[#6B645B]">
                        <div>
                            Showing
                            <span class="font-medium font-mono text-[#1C1916]">{{ products.from ?? 0 }}</span>
                            to
                            <span class="font-medium font-mono text-[#1C1916]">{{ products.to ?? 0 }}</span>
                            of
                            <span class="font-medium font-mono text-[#1C1916]">{{ products.total }}</span>
                            products
                        </div>
                    ...
                    <div v-if="products.links && products.links.length > 3" class="flex items-center gap-1">
        ...
        >
            <div class="w-full max-w-lg rounded-lg border border-[#E5E0D8] bg-white p-6 shadow-xl">
                ...
                <div class="mt-4 space-y-3">
                    <div class="rounded border border-[#F0D5C7] bg-[#FDF8F5] p-3 text-[12.5px] text-[#842C12]">
                        <p class="font-medium">Masterdata Form Notice</p>
                        <p class="mt-0.5 text-[12px] opacity-90">
                            As requested, this button is active for Admin users. The full product creation form with automated SKU generation and factor calculations will be connected in the masterdata form iteration.
                        </p>
                    </div>

                    <div class="grid grid-cols-2 gap-3 pt-2 text-[12.5px]">
                        <div>
                            <label class="block font-medium text-[#6B645B]">SKU Pattern</label>
                            <input
                                type="text"
                                disabled
                        ...
                        <div>
                            <label class="block font-medium text-[#6B645B]">Tile Brand</label>
                            <input
                                type="text"
                                disabled
                                value="Select Brand..."
                        ...
                        <div class="col-span-2">
                            <label class="block font-medium text-[#6B645B]">Tile Name (English & Bengali)</label>
                            <input
                                type="text"
                                disabled
                        ...
                        <div>
                            <label class="block font-medium text-[#6B645B]">Pieces per Box</label>
                            <input
                                type="text"
                                disabled
                                value="4 pcs"
                        ...
                        <div>
                            <label class="block font-medium text-[#6B645B]">Box Price (৳)</label>
                            <input
                                type="text"
                                disabled
                                value="৳ 0.00"
        ...
        >
            <div class="w-full max-w-md rounded-lg border border-[#E5E0D8] bg-white p-6 shadow-xl">
                ...
                <div class="mt-4 space-y-2.5 text-[13px]">
                    <div class="flex justify-between border-b border-[#F0ECE4] py-1">
                        <span class="text-[#6B645B]">SKU:</span>
                        <span class="font-mono font-semibold text-[#1C1916]">{{ activeProduct.sku }}</span>
                    </div>
                    <div class="flex justify-between border-b border-[#F0ECE4] py-1">
                        <span class="text-[#6B645B]">Product Name:</span>
                        <span class="font-medium text-[#1C1916]">{{ activeProduct.name }}</span>
                    </div>
                    <div class="flex justify-between border-b border-[#F0ECE4] py-1">
                        <span class="text-[#6B645B]">Brand:</span>
                        <span class="text-[#1C1916]">{{ activeProduct.brand?.name ?? 'None' }}</span>
                    </div>
                    <div class="flex justify-between border-b border-[#F0ECE4] py-1">
                        <span class="text-[#6B645B]">Packaging:</span>
                        <span class="font-mono text-[#1C1916]">{{ activeProduct.pieces_per_box }} pcs / {{ formatNumber(activeProduct.sqft_per_box, 2) }} sqft</span>
                    </div>
                    <div class="flex justify-between border-b border-[#F0ECE4] py-1">
                        <span class="text-[#6B645B]">Box Price:</span>
                        <span class="font-mono font-semibold text-[#B44422]">{{ formatCurrency(getBoxPrice(activeProduct)) }}</span>
                    </div>

                    <div class="mt-3 rounded border border-[#E5E0D8] bg-[#F7F5F0] p-2.5 text-[12px] text-[#6B645B]">
                        Admin action button is active. Form fields editing will be wired in the masterdata form step.
                    </div>
                </div>

                <div class="mt-5 flex items-center justify-end gap-2 border-t border-[#E5E0D8] pt-3">
                    <button
                        type="button"
                        @click="closeModals"
        ...
        >
            <div class="w-full max-w-lg rounded-lg border border-[#E5E0D8] bg-white p-6 shadow-xl">
                ...
                <div class="mt-4 space-y-3 text-[13px]">
                    <div class="grid grid-cols-2 gap-2 rounded border border-[#E5E0D8] bg-[#FAF8F5] p-3">
                        <div>
                            <span class="text-[11px] uppercase tracking-wider text-[#8F887C]">SKU Code</span>
                            <div class="font-mono font-semibold text-[#1C1916]">{{ activeProduct.sku }}</div>
                        </div>
                        <div>
                            <span class="text-[11px] uppercase tracking-wider text-[#8F887C]">Barcode</span>
                            <div class="font-mono text-[#1C1916]">{{ activeProduct.barcode ?? 'None' }}</div>
                        </div>
                        <div class="mt-2">
                            <span class="text-[11px] uppercase tracking-wider text-[#8F887C]">Brand</span>
                            <div class="font-medium text-[#1C1916]">{{ activeProduct.brand?.name ?? 'Standard' }}</div>
                        </div>
                        <div class="mt-2">
                            <span class="text-[11px] uppercase tracking-wider text-[#8F887C]">Tile Size</span>
                            <div class="font-medium text-[#1C1916]">{{ activeProduct.tile_size?.label ?? 'N/A' }} mm</div>
                        </div>
                    </div>

                    <!-- Stock Triple Highlight -->
                    <div class="rounded border border-[#E5E0D8] bg-[#F7F5F0] p-3">
                        <div class="text-[11px] font-semibold uppercase tracking-wider text-[#6B645B]">
                            Warehouse Aggregated On-Hand Stock
                        </div>
                        <div class="mt-2 grid grid-cols-3 gap-2 text-center">
                            <div class="rounded bg-white p-2 border border-[#E5E0D8]">
                                <div class="font-mono text-[14px] font-bold text-[#1C1916]">
                                    {{ getStockTriple(activeProduct).boxes }}
                                </div>
                                <div class="text-[10px] text-[#6B645B]">BOXES</div>
                            </div>
                            <div class="rounded bg-white p-2 border border-[#E5E0D8]">
                                <div class="font-mono text-[14px] font-bold text-[#1C1916]">
                                    {{ getStockTriple(activeProduct).pieces }}
                                </div>
                                <div class="text-[10px] text-[#6B645B]">PIECES</div>
                            </div>
                            <div class="rounded bg-white p-2 border border-[#E5E0D8]">
                                <div class="font-mono text-[14px] font-bold text-[#B44422]">
                                    {{ getStockTriple(activeProduct).sqft }}
                                </div>
                                <div class="text-[10px] text-[#6B645B]">SQFT</div>
                            </div>
                        </div>
                    </div>

                    <!-- Pricing Info -->
                    <div class="flex items-center justify-between rounded border border-[#E5E0D8] bg-white p-3">
                        <div>
                            <span class="text-[11px] text-[#8F887C]">Retail Box Price</span>
                            <div class="font-mono text-[15px] font-bold text-[#1C1916]">
                                {{ formatCurrency(getBoxPrice(activeProduct)) }}
                            </div>
                        </div>
                        <div class="text-right">
                            <span class="text-[11px] text-[#8F887C]">Equivalent Sqft Rate</span>
                            <div class="font-mono text-[15px] font-bold text-[#B44422]">
                ...
                <div class="mt-5 flex justify-end border-t border-[#E5E0D8] pt-3">
                    <button
                        type="button"
                        @click="closeModals"
